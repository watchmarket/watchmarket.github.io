...
(function() {
     ...
     * ✅ Using event delegation to ensure handler works even if element loads after script
     */
    $(document).on('click', '#UpdateWalletCEX', async function(e) {
        console.log('[WALLET HANDLER] UpdateWalletCEX clicked!'); // DEBUG

        // NEW UI: Show wallet exchanger section instead of running immediately
        try {
            console.log('[WALLET HANDLER] Checking window.App.WalletExchanger...', window.App?.WalletExchanger); // DEBUG
            if (window.App?.WalletExchanger?.show) {
                console.log('[WALLET HANDLER] Calling window.App.WalletExchanger.show()...'); // DEBUG
                window.App.WalletExchanger.show();
                console.log('[WALLET HANDLER] WalletExchanger.show() completed!'); // DEBUG
                return;
            } else {
                console.warn('[WALLET HANDLER] window.App.WalletExchanger.show not available, using fallback'); // DEBUG
            }
        } catch(err) {
            console.error('[WALLET HANDLER] Error showing wallet exchanger section:', err);
        }
