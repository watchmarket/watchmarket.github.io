...
// =================================================================================
// MODAL PROFILE MANAGEMENT - PER CHAIN
// =================================================================================
/**
 * Get modal profile storage key for a chain
 * @param {string} chainKey - Chain identifier (e.g., 'bsc', 'eth')
 * @returns {string} Storage key (e.g., 'MODAL_PROFILE_BSC')
 */
window.getModalProfileKey = function (chainKey) {
    const chain = String(chainKey || '').toUpperCase();
    return `MODAL_PROFILE_${chain}`;
};

/**
 * Load modal profiles for a chain
...
window.loadModalProfiles = async function (chainKey) {
    if (!chainKey || String(chainKey).toLowerCase() === 'multichain') {
        console.warn('[Modal Profile] Cannot load profiles for multichain mode');
    ...
    try {
        ...
        console.log(`[Modal Profile] Loaded ${result.length} profiles for chain: ${chainKey}`);
        return result;
    } catch (e) {
...
/**
 * Save modal profiles for a chain
 * @param {string} chainKey - Chain identifier
...
window.saveModalProfiles = async function (chainKey, profiles) {
    if (!chainKey || String(chainKey).toLowerCase() === 'multichain') {
        console.warn('[Modal Profile] Cannot save profiles for multichain mode');
        return false; // No save for multichain
    }

    try {
        if (!Array.isArray(profiles)) {
            console.error('[Modal Profile] Profiles must be an array');
            return false;
        }

        const key = window.getModalProfileKey(chainKey);
        console.log(`[Modal Profile] Saving ${profiles.length} profiles for chain: ${chainKey}`);
        ...
        if (window.__IDB_FLUSH_PENDING__) {
            ...
            console.log('[Modal Profile] ✅ Flushed to IndexedDB');
        }

        return true;
    } catch (e) {
        console.error('[Modal Profile] Error saving profiles:', e);
        return false;
    }
};

/**
 * Delete modal profiles for a chain
 * @param {string} chainKey - Chain identifier
 * @returns {Promise<boolean>} Success status
 */
window.deleteModalProfiles = async function (chainKey) {
    if (!chainKey || String(chainKey).toLowerCase() === 'multichain') {
        console.warn('[Modal Profile] Cannot delete profiles for multichain mode');
        return false;
    }

    try {
        const key = window.getModalProfileKey(chainKey);
        console.log(`[Modal Profile] Deleting all profiles for chain: ${chainKey}`);
        ...
        console.error('[Modal Profile] Error deleting profiles:', e);
        return false;
    }
};

/**
 * One-time migration from old keys (MODAL_PROFILES_) to new keys (MODAL_PROFILE_)
 * This preserves existing user profiles during the refactoring
 */
window.migrateOldModalProfiles = async function () {
    try {
        ...
        for (const chain of chains) {
            ...
            const oldKey = `MODAL_PROFILES_${chainUpper}`; // Old format (plural)
            const newKey = `MODAL_PROFILE_${chainUpper}`;  // New format (singular)

            // Check if old key exists
            const oldData = getFromLocalStorage(oldKey, null);
            if (oldData && Array.isArray(oldData) && oldData.length > 0) {
                // Check if new key already exists
                const newData = getFromLocalStorage(newKey, null);
                if (!newData) {
                    // Migrate
                    console.log(`[Migration] Migrating ${oldData.length} profiles from ${oldKey} to ${newKey}`);
            ...
            // Also migrate last profile index (remove old key)
            const oldLastKey = `MODAL_LAST_PROFILE_${chainUpper}`;
            try {
        ...
        if (migrated > 0) {
            console.log(`[Migration] ✅ Migrated ${migrated} profile sets`);
...
// Auto-run migration once on app load
if (typeof window !== 'undefined') {
    try {
        window.addEventListener('DOMContentLoaded', function () {
            setTimeout(function () {
                if (window.migrateOldModalProfiles) {
                    window.migrateOldModalProfiles();
                }
            }, 1000);
        });
    } catch (_) { }
